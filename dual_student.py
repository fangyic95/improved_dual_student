...
def create_model_r(name, num_classes, ema=False):
    LOG.info('=> creating {pretrained} {name} model: {arch}'.format(
        pretrained='pre-trained' if args.pretrained else 'non-pre-trained',
        name=name,
        arch=args.arch_r))
        ...
        if args.stabilization_loss_switch:
            ...
            if args.exclude_unlabeled:
                if args.cnnfeature2stable:
                    r_stabilization_loss = stabilization_weight * stabilization_criterion(r_cnnfeature, tar_l_cnnfeature) / minibatch_size
                else:
                ...
                if args.cnnfeature2stable:
                    r_stabilization_loss = stabilization_weight * stabilization_criterion(r_cnnfeature, tar_l_cnnfeature) / unlabeled_minibatch_size
                else:
...
def main(context):
    ...
    if args.arch_r == 'None':
        ...
        r_model = create_model_r(name='r', num_classes=num_classes)
    ...
    if args.resume_r:
        assert os.path.isfile(args.resume_r), '=> no checkpoint found at: {}'.format(args.resume_r)
        LOG.info('=> loading r model checkpoint: {}'.format(args.resume_r))

        checkpoint = torch.load(args.resume_r)
        # models and optimizers
        r_model.load_state_dict(checkpoint['r_model'])
        r_optimizer.load_state_dict(checkpoint['r_optimizer'])

        LOG.info('=> loaded r model checkpoint {} (epoch {})'.format(args.resume_r, checkpoint['epoch']))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.validation_epochs and (epoch + 1) % args.validation_epochs == 0:
            ...
            is_best_l = l_prec1 > best_prec1_l
            is_best_r = r_prec1 > best_prec1_r
